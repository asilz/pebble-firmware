...
static void prv_restore_rtc_time_state(void) {
  ...
  if (s_had_amnesia_on_boot) {
    ...
    PBL_LOG(LOG_LEVEL_INFO, "Restore RTC: we are on our way up with interval_ticks = %" PRIu32,
            current_ticks);
    PBL_LOG(LOG_LEVEL_INFO, "Restore RTC: saved: %" PRIu32 " diff: %" PRIu32, last_save_time_ticks,
            ticks_since_last_save);
  }

  char buffer[TIME_STRING_BUFFER_SIZE];
  PBL_LOG(LOG_LEVEL_INFO, "Restore RTC: saved_time: %s raw: %lu",
          time_t_to_string(buffer, last_save_time), last_save_time);
...
bool rtc_alarm_is_initialized(void) { return 0; }
